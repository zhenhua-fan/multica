...
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, wiki_enabled, wiki_top_k, wiki_threshold
`
...
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, wiki_enabled, wiki_top_k, wiki_threshold FROM agent
WHERE id = $1
`
...
UPDATE agent SET
    ...
    wiki_enabled = COALESCE($16, wiki_enabled),
    wiki_top_k = COALESCE($17, wiki_top_k),
    wiki_threshold = COALESCE($18, wiki_threshold),
    updated_at = now()
WHERE id = $1
RETURNING id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, wiki_enabled, wiki_top_k, wiki_threshold
`
...
type UpdateAgentParams struct {
	...
	McpConfig          []byte        `json:"mcp_config"`
	Model              pgtype.Text   `json:"model"`
	WikiEnabled        pgtype.Bool   `json:"wiki_enabled"`
	WikiTopK           pgtype.Int4   `json:"wiki_top_k"`
	WikiThreshold      pgtype.Float4 `json:"wiki_threshold"`
}
